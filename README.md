...
1. ผู้จองเลือก **รอบการแสดง** ก่อน (4 รอบตาม `SHOWTIMES` ใน `config.js`) จากนั้นหน้า **`public/index.html`**
   จะโหลดผังที่นั่งจาก `config.js` แล้วขอสถานะที่นั่งล่าสุดของทุกรอบจาก Apps Script (`action=seats`) มาทับ
   cache ที่เก็บไว้ใน `localStorage` ของเบราว์เซอร์ — ทำให้ผังขึ้นเร็วและใช้ได้แม้เน็ตช้า แล้ว sync ซ้ำทุก ~15 วิ
   ที่นั่งเดียวกันคนละรอบจองแยกกันได้อิสระ (ล็อกแยกตามรอบ)
2. ผู้ใช้เลือกที่นั่งของรอบที่เลือกไว้ กรอกฟอร์ม แนบสลิป กด "ยืนยันการจอง" → บันทึกลง `localStorage` ทันที
   (optimistic) พร้อมส่งไป Apps Script (`action=create`) ซึ่งตรวจที่นั่งชนกันซ้ำอีกครั้งฝั่งเซิร์ฟเวอร์
   เฉพาะภายในรอบเดียวกัน (กันสองคนจองที่เดียวกันพร้อมกัน) แล้วอัปโหลดไฟล์สลิปขึ้น Google Drive โดยอัตโนมัติ
...
4. **`public/admin.html`** ล็อกอินด้วยรหัสผ่าน (ตรวจสอบฝั่ง Apps Script ไม่ใช่ฝั่งเว็บ) เห็นรายการจองทั้งหมด
   พร้อมรูปสลิป มีช่องกรองตามรอบการแสดง/สถานะ + ช่องค้นหาเลขที่การจอง (ใช้เช็คหน้างาน) และปุ่มอนุมัติ/ปฏิเสธ
...
| `SHOW_INFO` | ชื่อการแสดง / สถานที่ / คำโปรยสรุปวันเวลาที่โชว์บนหัวเว็บ | ✅ แก้ข้อความได้อิสระ |
| `SHOWTIMES` | รายการรอบการแสดงที่เปิดให้จอง แต่ละรอบมี `id` + `label` | ⚠️ เพิ่ม/แก้ข้อความ `label` ได้อิสระ แต่ **ห้ามแก้/ลบ `id`** ของรอบที่เคยมีคนจองแล้ว เพราะผูกกับการล็อกที่นั่งใน Sheets อยู่ (เพิ่มรอบใหม่ต่อท้ายได้เสมอ) |
...
first_name, last_name, phone, payment_method, payment_amount_declared, slip_drive_file_id, slip_url,
booking_status, reject_reason, updated_at`

- `showtime_id` = รหัสรอบการแสดง (อ้างอิง `id` ใน `SHOWTIMES`), `showtime_label` = ข้อความรอบที่อ่านง่าย
- `seats` = รหัสที่นั่งคั่นด้วย `|` (อ้างอิง `id` ใน `SEAT_LAYOUT`) — **ที่นั่งเดียวกันคนละรอบ (`showtime_id` ต่างกัน)
  จองซ้อนกันได้** เพราะระบบล็อกที่นั่งแยกตามรอบ ไม่ใช่ล็อกรวมทั้งชีต
...
- กันที่นั่งจองซ้ำที่ **ฝั่งเซิร์ฟเวอร์เป็นหลัก** (`LockService` ใน Code.gs) การเช็กฝั่งหน้าเว็บเป็นแค่ UX —
  เช็กเฉพาะ**ภายในรอบการแสดงเดียวกัน** ที่นั่งเดียวกันต่างรอบไม่ชนกัน
